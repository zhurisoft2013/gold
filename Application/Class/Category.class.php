...
class Category {
    ...
    public function getChildIds($pid=0){
      $field=array($this->fields['cid'], $this->fields['fid']);
      $category=$this->model->field($field)->select();
      $list=$this->getChildsId($category,$pid);
      return $list;
    }
    private function getChildsId($category,$pid=0){
      $arr=array();
      foreach ($category as $value) {
        if ($value['pid']==$pid) {
          $arr[]=$value['id'];
          $arr=array_merge($arr,$this->getChildsId($category,$value['id']));
        }
      }
      return $arr;
}

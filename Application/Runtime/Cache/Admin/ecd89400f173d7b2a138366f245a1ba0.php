...
<dd><a href="<?php echo U('Admin/Message/index');?>"><em class="menu-icon icon-list"></em>留言列表</a></dd> -->
	...
	<div class="FormWrap">
		<h3>编辑文章</h3>
		<div class="division">
			<form action="<?php echo U('Admin/Article/update');?>" method="post" id="frmEditArticle">
				<table class="liststyle data" width="100%">
					<tbody>
						<tr>
							<th>标题</th>
							<td>
								<input type="text" name="title" id="title" class="half" value="<?php echo ($article["title"]); ?>" />
							...
							<td>
								<select name="category_id" id="category" >
									...
									<?php if(is_array($category)): foreach($category as $key=>$vo): ?><option value="<?php echo ($vo["id"]); ?>" 
										<?php if(($article['category_id']) == $vo['id']): ?>selected="selected"<?php endif; ?>
						...
						<tr>
							<th>是否置顶</th>
							<td>
								<label><input type="checkbox" name="top" value="1" <?php if(($article['top']) == "1"): ?>checked="checked"<?php endif; ?> />置顶</label>
							</td>
						</tr>
						<tr>
							<th>文章排序</th>
							<td>
								<input type="text" name="sort" value="100" value="<?php echo ($article["sort"]); ?>" />
							</td>
						</tr>
						<tr>
							<th>文章内容</th>
							<td>
								<script id="editor" type="text/plain" name="content" ><?php echo ($article["content"]); ?></script>
							</td>
						</tr>
						<tr>

...
<dd><a href="<?php echo U('Admin/Message/index');?>"><em class="menu-icon icon-list"></em>留言列表</a></dd> -->
	...
	<div class="FormWrap">
		<h3>添加分类</h3>
		<div class="division">
			<form action="<?php echo U('Admin/Category/addHandle');?>" method="post" id="frmAddCategory">
				<table class="liststyle data" width="100%">
					<tbody>
						<tr>
							...
							<select name="pid" id="cid">
								...
								<?php if(is_array($category)): foreach($category as $key=>$vo): ?><option value="<?php echo ($vo["id"]); ?>" 
									<?php if(($pid) == $vo['id']): ?>selected="selected"<?php endif; ?>
						...
						<tr id="menu" <?php if(($pid) != "0"): ?>style="display:none;"<?php endif; if(!isset($pid)): ?>style="display:none;"<?php endif; ?> >
							<th>是否显示在首页菜单</th>
							<td>
								<label><input type="checkbox" name="show_menu" id="show_menu" value="1" >作为首页菜单项</label>
							</td>
						</tr>						
						<tr>

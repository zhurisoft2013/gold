...
<dd><a href="<?php echo U('Admin/Message/index');?>"><em class="menu-icon icon-list"></em>留言列表</a></dd> -->
	...
	<h3 >文章列表<form action="<?php echo U('Admin/Article/index');?>" method="post" class="frmFind">
		按类别查看：<select name="category_id" id="category" >
			<option value="0">所有文章</option>
			<?php if(is_array($category)): foreach($category as $key=>$vo): ?><option value="<?php echo ($vo["id"]); ?>"><?php echo ($vo["fullname"]); ?></option><?php endforeach; endif; ?>
...
<!-- 	<table class="liststyle full">
		...
		<tbody>
			<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
					...
					<td>
						<a href="<?php echo U('Admin/Article/edit',array('id'=>$vo['id']));?>" class="abtn"><em class="btn-icon icon-edit"></em>修改文章</a>&nbsp;
						<a onclick="javascript:return p_del()" href="<?php echo U('Admin/Article/delete',array('id'=>$vo['id']));?>" class="abtn"><em class="btn-icon icon-trash"></em>删除文章</a>
						<a href="<?php echo U('Home/Index/article',array('id'=>$vo['id']));?>" class="abtn" target="_blank"><em class="btn-icon  icon-list-alt"></em>查看文章</a>					

					</td>
				</tr><?php endforeach; endif; else: echo "" ;endif; ?>
			<tr>
				<td colspan="6"><div class="manu"><?php echo ($pageinfo); ?></div></td>
			</tr>
		</tbody>
	</table>	
